...
import com.rkukharenka.telegrambot.instaboxbot.enums.CalendarState;
...
public final class KeyboardFactory {
    ...
    public static ReplyKeyboard getCalendar(YearMonth yearMonth) {
        ...
        fillCalendarButtons(yearMonth, buttonRows, calendarArray);
    ...
    private static void fillCalendarButtons(YearMonth yearMonth, List<List<InlineKeyboardButton>> buttonRows, int[][] calendarArray) {
        for (int[] weekArray : calendarArray) {
            ...
            for (int dayOfMonth : weekArray) {
                if (dayOfMonth == 0) {
                    ...
                    week.add(createInlineButton(String.valueOf(dayOfMonth),
                            ChatUtils.formatDateToString(LocalDate.of(yearMonth.getYear(), yearMonth.getMonth(), dayOfMonth))));
                }
            }
            buttonRows.add(week);
        }
